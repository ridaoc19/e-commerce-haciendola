...
function useAdvertising(): UseAdvertising {
  ...
  return {
    ...
    ShowcaseProductView: topViewedProducts.length > 0 ? <Showcase title='Lo mas visto' products={topViewedProducts.map(({ images, price, product, product_id, listPrice}) => {
      return {
        product_id, images, price, product, listPrice
      }
    })} /> : null,
  };
}
